...
- 增加 A/B 对照实验面板，让两次 preview / retrieval 状态可以并排比较并高亮差异词
...
- 提供 A/B 对照实验面板，允许把当前 preview / retrieval 状态固定到两个槽位后直接比较差异，并高亮 query 命中词和槽位独有词
...
- 如果当前已经跑过检索，也会把 query、threshold、结果数和前两条命中一起固定下来
